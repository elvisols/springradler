...
import static com.springapp.springradler.config.CacheConfiguration.ITEMS_CACHE;
...
public class ItemControllerIntegrationTest extends AbstractIntegrationTest {
    ...
    public void shouldGetItem_FromCache() {
        ...
        final HttpEntity<CreateItemDTO> createEntity = new HttpEntity<>(new CreateItemDTO("new item for cache"),
                httpHeaders());

        ResponseEntity<Item> createdItem = restTemplate.exchange(url("/items"), HttpMethod.POST, createEntity,
                Item.class);
        ...
        final ResponseEntity<Item> foundItem = restTemplate.exchange(url("/items/" + itemId), HttpMethod.GET,
                voidEntity, Item.class);
        assertThat(foundItem.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(foundItem.getBody()).isNotNull();
        ...
        final ResponseEntity<Void> deletedItem = restTemplate.exchange(url("/items/" + itemId), HttpMethod.DELETE,
                voidEntity, Void.class);
    ...
    public void shouldFindAllItems() throws JsonProcessingException {
        ...
        ResponseEntity<String> foundItemsPage = restTemplate.exchange(url("/items?page=0&size=1"), HttpMethod.GET,
                entity, String.class);
        assertThat(foundItemsPage.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(foundItemsPage.getBody()).isNotNull();

        PageImpl<Item> foundItems = objectMapper.readValue(foundItemsPage.getBody(),
    ...
    public void shouldCreateItem() {
        final HttpEntity<CreateItemDTO> entity = new HttpEntity<>(new CreateItemDTO("new item"), httpHeaders());

        ResponseEntity<Item> createdItem = restTemplate.exchange(url("/items"), HttpMethod.POST, entity, Item.class);
    ...
    public void shouldNotCreateItem_LongTitle() {
        final String title = Stream.generate(() -> "a").limit(257)
                .collect(StringBuilder::new, StringBuilder::append, StringBuilder::append).toString();

        final HttpEntity<CreateItemDTO> entity = new HttpEntity<>(new CreateItemDTO(title), httpHeaders());

        ResponseEntity<ErrorsDTO> error = restTemplate.exchange(url("/items"), HttpMethod.POST, entity,
                ErrorsDTO.class);
    ...
    public void shouldUpdateItem() {
        final HttpEntity<UpdateItemDTO> entity = new HttpEntity<>(new UpdateItemDTO("updated item"), httpHeaders());

        ResponseEntity<Item> createdItem = restTemplate.exchange(url("/items/2"), HttpMethod.PUT, entity, Item.class);
    ...
    public void shouldDeleteItem() {
        ...
        ResponseEntity<Item> createdItem = restTemplate.exchange(url("/items/2"), HttpMethod.DELETE, entity,
                Item.class);
